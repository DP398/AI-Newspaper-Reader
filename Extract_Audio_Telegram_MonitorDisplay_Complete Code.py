...
import pygame  # For playing audio and handling keyboard events
...
# Initialize pygame for audio playback and event handling
pygame.mixer.init()
...
def send_audio_response(update: Update, context: CallbackContext, response: str):
    ...
    # Display audio file information on the monitor
    print(f"Playing audio: {audio_file}")
    cv2.imshow('Audio Playback', cv2.imread('audio_icon.jpg'))  # You can add a custom icon for audio

    # Play the audio on the monitor with skip feature
    pygame.mixer.music.load(audio_file)
    pygame.mixer.music.play()

    # Display message and wait for a key to skip or let it play
    print("Press any key to skip audio playback...")
    skip_audio = False

    while pygame.mixer.music.get_busy():  # Wait until the audio finishes playing
        if skip_audio or cv2.waitKey(1) != -1:  # Detect keypress to skip
